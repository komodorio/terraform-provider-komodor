...
func (c *Client) CreateWorkspace(workspace *NewWorkspace) (*Workspace, error) {
	...
	resBody, statusCode, err := c.executeHttpRequest("POST", c.GetV2Endpoint()+"/workspaces", &body)
	if err != nil {
...
func (c *Client) GetWorkspace(id string) (*Workspace, int, error) {
	resBody, statusCode, err := c.executeHttpRequest("GET", fmt.Sprintf(c.GetV2Endpoint()+"/workspaces/%s", id), nil)
	if err != nil {
...
func (c *Client) UpdateWorkspace(id string, workspace *NewWorkspace) (*Workspace, error) {
	...
	resBody, statusCode, err := c.executeHttpRequest("PUT", fmt.Sprintf(c.GetV2Endpoint()+"/workspaces/%s", id), &body)
	if err != nil {
...
func (c *Client) DeleteWorkspace(id string) error {
	_, statusCode, err := c.executeHttpRequest("DELETE", fmt.Sprintf(c.GetV2Endpoint()+"/workspaces/%s", id), nil)
	if err != nil {
